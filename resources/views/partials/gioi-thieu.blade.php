        ...
        <div class="container mx-auto px-4">
            ...
            <div class="max-w-2xl mx-auto mb-8">
                <div class="relative group">
                    ...
                    {{-- Scattered Customer Icons --}}
                    {{-- North Region (Teal) --}}
                    <template x-for="(customer, index) in regions.north.customers.slice(0, 3)">
                        <div @click="openRegion('north')"
                            class="absolute w-8 h-8 md:w-10 md:h-10 rounded-full border-2 border-primary-500 bg-white shadow-lg cursor-pointer hover:scale-125 transition-transform z-10 overflow-hidden group"
                            :style="index === 0 ? 'top: 5%; left: 37%' : (index === 1 ? 'top: 10%; left: 47%' : 'top: 20%; left: 44%')"
                            :title="customer.name">
                    ...
                    <template x-for="(customer, index) in regions.central.customers.slice(0, 3)">
                        <div @click="openRegion('central')"
                            class="absolute w-8 h-8 md:w-10 md:h-10 rounded-full border-2 border-accent-500 bg-white shadow-lg cursor-pointer hover:scale-125 transition-transform z-10 overflow-hidden group"
                            :style="index === 0 ? 'top: 42%; left: 58%' : (index === 1 ? 'top: 53%; left: 62%' : 'top: 60%; left: 62%')"
                            :title="customer.name">
                            <img :src="customer.logo" :alt="customer.name" class="w-full h-full object-cover">
                            {{-- Tooltip --}}
                            <div class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-black/80 text-white text-xs rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none" x-text="customer.name"></div>
                    ...
                    <template x-for="(customer, index) in regions.south.customers.slice(0, 3)">
                        <div @click="openRegion('south')"
                            class="absolute w-8 h-8 md:w-10 md:h-10 rounded-full border-2 border-secondary-500 bg-white shadow-lg cursor-pointer hover:scale-125 transition-transform z-10 overflow-hidden group"
                            :style="index === 0 ? 'top: 68%; left: 59%' : (index === 1 ? 'top: 75%; left: 52%' : 'top: 80%; left: 44%')"
                            :title="customer.name">
                            <img :src="customer.logo" :alt="customer.name" class="w-full h-full object-cover">
                            {{-- Tooltip --}}
                            <div class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-black/80 text-white text-xs rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none" x-text="customer.name"></div>
